...
""",
    ...
    def test_android_reference_is_retired_only_when_the_checkout_has_no_android_tree(self) -> None:
        target = check_source_references.REPO_ROOT / "android/app/src/main/Foo.kt"
        self.assertEqual(
            not (check_source_references.REPO_ROOT / "android").is_dir(),
            check_source_references.target_is_retired_platform_source(target),
        )
        self.assertFalse(
            check_source_references.target_is_retired_platform_source(self.sources / "Foo.kt")
        )
