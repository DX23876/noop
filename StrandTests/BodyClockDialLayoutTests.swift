...
final class BodyClockDialLayoutTests: XCTestCase {
    ...
    func testTheHourNumeralsDoNotScaleWithDynamicType() throws {
        ...
        XCTAssertTrue(src.contains(".font(StrandFont.diagramLabel)"),
                      "dial numerals need the fixed diagram token, not a Dynamic Type text style")
        ...
        XCTAssertFalse(Self.codeLines(of: src).contains { $0.contains(".font(.system(size:") },
                       "screen code must use the StrandDesign typography token")
    }
