...
final class ReadSpineActiveDeviceTests: XCTestCase {
    ...
    /// hands `editSleepGroupTimes` id-free `CachedSleepSession`s, so the write must resolve that canonical
    /// owner (via `sleepOwnerIds`, through `ownedSleepRowSnapshot`) instead of trying only the active
    /// strap's two namespaces and silently no-op'ing — the "I corrected the night but it snapped back"
    /// report. The corrected window here only NARROWS, which is also the path that keeps the planner's
    /// cheap re-clip rather than re-staging from raw.
    @MainActor
    func testEditingCanonicalSleepAfterReAddPersistsTheVisibleNight() async throws {
        let store = try await WhoopStore.inMemory()
        try await store.upsertDevice(id: canonicalId, mac: nil, name: "WHOOP")
        try await store.upsertDevice(id: newId, mac: nil, name: "WHOOP")
        let night = CachedSleepSession(startTs: 1_000, endTs: 5_000, efficiency: 0.9,
                                       restingHr: 52, avgHrv: 60, stagesJSON: nil)
        _ = try await store.upsertSleepSessions([night], deviceId: canonicalId)
        ...
        let outcome = await repo.editSleepGroupTimes([night], newStartTs: 1_200, newEndTs: 4_800)
        guard case .success = outcome else {
            return XCTFail("the canonical night must resolve an owner and persist, got \(outcome)")
        }
