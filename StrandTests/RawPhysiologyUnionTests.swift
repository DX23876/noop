...
final class RawPhysiologyUnionTests: XCTestCase {
    ...
    @MainActor
    func testHrvTimelineKeepsLegacyBeatsAfterWhoop5RePairing() async throws {
        let store = try await WhoopStore.inMemory()
        let registry = DeviceRegistryStore(dbQueue: store.registryWriter)
        ...
        repo.setStoreForTesting(store)
        // This fork scores unlabelled legacy beats per beat rather than withholding them (#2117).
        let kept = await repo.timelineSeries(metric: .hrv, from: base, to: base + 600, targetPoints: 600)
        XCTAssertFalse(kept.points.isEmpty)
    ...
    func testWhoop5SwitchKeepsOlderPhysicalOwnersAndCapturedBeatOrder() async throws {
        ...
        // The canonical history's legacy beat is kept: nothing labelled covers ts 90.
        XCTAssertEqual(rows.map(\.rrMs), [1024, 810, 900, 700, 900, 800, 850])
