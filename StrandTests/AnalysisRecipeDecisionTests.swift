...
final class AnalysisRecipeDecisionTests: XCTestCase {
    ...
    /// witnessing one. This one names the numbers: an install carrying the v5 fusion recipe must ask
    /// for the v6 native-training migration, and it must ask for it as `5 → 6` rather than another pair.
    ///
    ...
    func testRecipeVersionIsSixAndAV5InstallMigratesToIt() {
        XCTAssertEqual(IntelligenceEngine.currentAnalysisRecipeVersion, 6,
                       "recipe version changed — answer 'Analysis migration required' for what moved")
        XCTAssertEqual(IntelligenceEngine.analysisRecipeDecision(storedVersion: 5),
                       .migrate(from: 5, to: 6))
    }
