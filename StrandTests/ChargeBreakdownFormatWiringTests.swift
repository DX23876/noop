...
/// "What shaped it" path had no test at all while Android's did. `ChargeBreakdownFormat.compute` is that
...
final class ChargeBreakdownFormatWiringTests: XCTestCase {
    ...
    func testRhrRowIsAbsentWhenTheRestingHrBaselineIsNotUsable() {
        ...
        let out = ChargeBreakdownFormat.compute(row: today, days: history + [today], restScore: 85)
    ...
    func testColdStartHistoryProducesNoBreakdown() {
        ...
        XCTAssertNil(ChargeBreakdownFormat.compute(row: days[1], days: days, restScore: 85))
    }
    ...
    func testAScoredDayProducesDriverRowsAndATier() {
        ...
        let out = ChargeBreakdownFormat.compute(row: today, days: past + [today], restScore: 85)
        XCTAssertNotNil(out)
        let labels = (out?.drivers ?? []).map(\.label)
    ...
    func testDriverBaselineHonoursTheRecalibrationEpoch() {
        ...
        let whole = ChargeBreakdownFormat.compute(row: today, days: days, restScore: 85,
                                                  hrvBaselineEpoch: 0)
        let fromEpoch = ChargeBreakdownFormat.compute(row: today, days: days, restScore: 85,
    ...
    func testAnAbsentEpochLeavesTheBaselineExactlyAsItWas() {
        ...
        let past = (1...10).map { day(String(format: "2026-01-%02d", $0), hrv: 50 + Double($0 % 3)) }
        let today = day("2026-01-20", hrv: 62, rhr: 51, recovery: 64)
        let implicit = ChargeBreakdownFormat.compute(row: today, days: past + [today], restScore: 85)
        let explicitZero = ChargeBreakdownFormat.compute(row: today, days: past + [today], restScore: 85,
                                                         hrvBaselineEpoch: 0)
