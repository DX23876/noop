...
final class HealthKitBridge: ObservableObject {
    ...
    private func writeVitals(whoopStore: WhoopStore, days: Int,
        ...
        func add(_ id: HKQuantityTypeIdentifier, _ unit: HKUnit, _ value: Double, _ day: String,
            ...
            // in Apple Health as unreachable duplicates. The natural key (`noop:<metric>:<day>`) is
            // stable across strap lifecycle changes.
            let key = HealthWriteback.appleHealthVitalKey(metricId: id.rawValue, day: day)
            // The origin marker stays beside the key: `isNoopAuthored` reads it to keep our own
            // write-backs out of the workouts/vitals we later READ from Apple Health, and the key
            // change must not quietly reopen that loop.
        ...
        for row in rows {
            ...
            // mislabelling problem upstream ryanbr/noop fixed in #1334.) A night without an SDNN also
            // retracts any sample an earlier build wrote for that day (#2264).
        ...
        for (workout, row) in workoutsAndRows {
            if let route = await Self.fetchWorkoutRoute(for: workout, store: store),
               route.count >= 2 {
                ...
                // Keyed by the stored row's own start and sport label, which is what the detail screen
                // looks the route up by (`HealthWorkoutActivityCatalog` names differ from `sportName`).
