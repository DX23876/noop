...
final class StepsDetailDensityTests: XCTestCase {
    ...
    /// The committed oracle, beside this file. Upstream keeps it under `android/app/src/test/resources/`
    /// for the Kotlin twin; this fork has no Android tree (`docs/FORK_GUIDE.md`), so it lives in `Oracles/`
    /// like `local_day_windows_oracle.json`. Fails rather than skips when absent.
    private func loadOracle() throws -> [[String: Any]] {
        let url = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .appendingPathComponent("Oracles/steps_detail_density_oracle.json")
