...
final class HrvDiagnosticRowsTests: XCTestCase {
    ...
    /// Night: two beats every second (refused). Nap: one per second (clean). The night's beats carry a
    /// transport, because this fork keeps a wholly provenance-less night as unverified instead of refusing it.
    private func dayResult() -> (AnalyticsEngine.DayResult, [RRInterval]) {
        let night = (nightStart..<nightEnd).flatMap {
            [RRInterval(ts: $0, rrMs: 880, transport: .whoopHistorical),
