...
final class LocalDayWindowsTests: XCTestCase {
    ...
    // MARK: - The committed oracle

    /// The committed oracle, beside this file.
    ///
    /// Upstream keeps it under `android/app/src/test/resources/` so a Kotlin twin asserts the same
    /// bytes. This fork has no Android tree (`docs/FORK_GUIDE.md`), so the cross-platform half of the
    /// contract is gone — but the file itself is not just a parity token: it is 19 recorded starts,
    /// their windows, day keys and sleep-window ends, produced once and committed, and re-deriving
    /// them is what catches a change in this helper's zone handling. It therefore moves here rather
    /// than being deleted with the tree it lived in.
    ///
    /// It fails rather than skips when the file is absent: an oracle nobody can find would otherwise
    /// let the suite stay green while the helper drifts, which is the whole thing this file guards.
    private func loadOracle() throws -> [String: Any] {
        let url = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .appendingPathComponent("Oracles/local_day_windows_oracle.json")
        guard FileManager.default.fileExists(atPath: url.path) else {
            XCTFail("committed oracle \(url.path) not found — this test must not pass by default")
