...
final class AnalyticsEngineRefusedMainNightHrvTests: XCTestCase {
    ...
    /// windows DO yield an RMSSD — the shape of a ring night scored from two beat channels at once. The
    /// beats carry a transport: in this fork a wholly provenance-less night is kept as unverified rather
    /// than refused (`SleepStager.sessionHrvOverCounted`), so a refused night needs tagged data.
    private func overCountedRR(_ start: Int, _ end: Int) -> [RRInterval] {
        (start..<end).flatMap { [RRInterval(ts: $0, rrMs: 880, transport: .whoopHistorical),
                                 RRInterval(ts: $0, rrMs: 960, transport: .whoopHistorical)] }
    }
