...
final class Whoop5RRStoreTests: XCTestCase {
    ...
    /// The diagnostic `rrTransport` line's "labelled" date: labelled v18 / standard channels only, suspect
    /// stamps excluded, and nil rather than a fabricated epoch when nothing labelled has been banked yet.
    func testFirstLabelledTimestampCountsOnlyLabelledChannels() async throws {
        let store = try await WhoopStore.inMemory()
        try registry(store, model: "5.0 MG")
        // Legacy unlabelled beats only: nothing labelled yet.
        _ = try await store.insert(Streams(rr: (100..<110).map { RRInterval(ts: $0, rrMs: 1000) }), deviceId: id)
        var first = try await store.firstLabelledWhoop5RRTimestamp(deviceId: id)
        XCTAssertNil(first)
        // The lower bound sees those same rows: they were recorded.
        var recorded = try await store.firstRecordedRRTimestamp(deviceId: id)
        XCTAssertEqual(recorded, 100)
        // A type-40 live beat (6) does not start labelled history, so it must not count.
        _ = try await store.insert(Streams(rr: [RRInterval(ts: 200, rrMs: 1000, srcChannel: .whoop5Realtime)]),
                                   deviceId: id)
        first = try await store.firstLabelledWhoop5RRTimestamp(deviceId: id)
        XCTAssertNil(first)
        ...
        first = try await store.firstLabelledWhoop5RRTimestamp(deviceId: id)
        XCTAssertNil(first)
        // The first labelled beat, and then an earlier one, which must win.
        ...
        let other = try await store.firstLabelledWhoop5RRTimestamp(deviceId: "someone-else")
        XCTAssertNil(other)
        let otherRecorded = try await store.firstRecordedRRTimestamp(deviceId: "someone-else")
        XCTAssertNil(otherRecorded)
    }

    func testHistoricalCanonicalOwnerAfterRePairingKeepsLegacyBeats() async throws {
        let store = try await WhoopStore.inMemory()
        ...
        let kept = try await store.rrIntervals(deviceId: "my-whoop", from: 0, to: 1000, limit: 100,
                                              unlabelledAliasOfWhoop5: true)
        XCTAssertEqual(kept.map(\.rrMs), [1024], "unlabelled WHOOP 5 alias history is read, never withheld")
    ...
    func testOrdinaryReadsAndFingerprintsFollowActiveAliasPolicy() async throws {
        ...
        let kept = try await read(store)
        XCTAssertEqual(kept.map(\.rrMs), [1000], "a WHOOP 5 alias keeps its legacy beats")
    ...
    func testOwnerPolicyKeepsLegacyAndPrefersLabelledPerBeatWithoutChangingStoredRows() async throws {
        ...
        XCTAssertEqual(rows.map(\.rrMs), [1000], "legacy beats nothing labelled covers are kept")
        ...
        rows = try await read(s)
        XCTAssertEqual(rows.map(\.rrMs), [977], "the labelled beat one second away replaces the legacy one")
    ...
    func testFirstTagOutsideDayAndSuspectPromotionInvalidateOwnerPolicyCachesWithoutDroppingLegacy() async throws {
        for suspect in [false, true] {
            ...
            XCTAssertEqual(after.map(\.rrMs), [800], "a tag outside the window never drops the legacy beat inside it")
            let g1 = try await s.analysisFingerprint()
            let d1 = try await s.dayStreamFingerprint(deviceId: id, from: 0, to: 1000)
