...
final class MetricsCacheTests: XCTestCase {
    ...
    func testSleepGroupEditRollsBackEveryFragmentWhenAnyKeyIsMissing() async throws {
        ...
        let stored = try await store.sleepSessions(deviceId: "devA", from: 0, to: 10_000, limit: 10)
        // #2039: a stored session carries the device it was read from.
        let expected = CachedSleepSession(startTs: 1_000, endTs: 2_000, efficiency: nil, restingHr: nil,
                                          avgHrv: nil, stagesJSON: "[\"original\"]", deviceId: "devA")
        XCTAssertEqual(stored, [expected], "the first mutation must roll back with the missing second row")
    }
