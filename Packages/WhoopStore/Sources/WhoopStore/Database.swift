...
extension WhoopStore {
    ...
    static func makeMigrator() -> DatabaseMigrator {
        ...
        // v68 (upstream v45). Source promotions change scoring without adding rows. Cover their cache witnesses so
        // legacy/non-WHOOP installs do not scan the entire R-R table on every analysis tick.
        migrator.registerMigration("v68-rr-source-index") { db in
            try db.create(index: "rrInterval_source_suspect", on: "rrInterval", columns: ["srcChannel", "tsSuspect"])
        }
        // v69-lift-log (upstream v46): the in-app strength log — saved programs and the sessions run from them.
        //
        ...
        migrator.registerMigration("v69-lift-log") { db in
