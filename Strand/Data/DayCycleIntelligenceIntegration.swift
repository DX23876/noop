...
actor DayCycleIntelligenceIntegration {
    ...
    func compute(nights: [Night], editedRows: [CachedSleepSession], store: WhoopStore,
        ...
        windowLoop: for (windowIndex, window) in windows.enumerated() {
            ...
            if !activityNeedsRecompute, let persisted {
                ...
                // A day the fingerprint reuse above cannot skip may still hold a cycle whose heart rate did
                // not move since the last pass (#2293): its onset or the next day changed, not its beats.
                // The index-only count and newest timestamp per owner witness the heart rate the same way
                // the day cache does, so an unchanged cycle is scored once instead of re-read every pass.
                var hrWitness: [String] = []
                if hrEndInclusive >= window.onset {
                    for owner in owners {
                        let fp = try? await store.hrFingerprint(deviceId: owner, from: window.onset,
                                                                to: hrEndInclusive)
                ...
                let load: CachedLoad
                if let hit = cache.loads[window.sleepId], hit.key == loadKey,
                   !hrWitness.contains(where: { $0.hasSuffix("=unread") }) {
                    load = hit
                } else {
    ...
    static func applying(_ result: Result, to daily: DailyMetric) -> DailyMetric {
        ...
        return DailyMetric(day: daily.day, totalSleepMin: daily.totalSleepMin, efficiency: daily.efficiency,
            ...
            // Carried, not defaulted: this rebuilder substitutes four activity fields and nothing else,
            // so a column it forgets to pass is silently nulled on every day the attribution touches.
