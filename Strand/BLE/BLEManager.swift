...
public final class BLEManager: NSObject, ObservableObject {
    ...
    /// Retry count for WHOOP 4 GET_CLOCK when correlation has not arrived before backfill starts.
    ...
    nonisolated static let pendingManualSyncTTL: TimeInterval = 600   // 10 min
...
extension BLEManager: @preconcurrency CBPeripheralDelegate {
    ...
    /// two would share `.whoop5`, and a raw-value compare would then reset on a same-family switch.
