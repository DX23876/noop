...
//
// NOOP is offline by design. This is the only feature that may send health context: when the user pastes their OWN
...
final class AICoachEngine: ObservableObject {
    ...
    // MARK: - The scheduled brief's hand-off into the transcript
    //
    // Upstream's K2 arrived here with its own SQLite-backed transcript (`CoachMessageStore`, a
    // `coachMessage` table and a `persistMessages()` after every turn). This fork already persists the
    // whole multi-conversation history through `CoachConversationStore` on the `transcriptAutosave`
    // pipeline, so that second store is not adopted: two writers for one transcript is exactly the
    // "second source of truth" a merge is supposed to prevent. What IS adopted is the pair below —
    // the seam a brief generated OUTSIDE the chat needs to land in it.
