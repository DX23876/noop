...
protocol StreamingToolClient {
    ...
    /// final assistant text. When `tools` is non-empty the model may request tools mid-stream; `runTool`
    /// executes a call by name and its result is fed back for the next round. `onDelta` runs on the main
    /// actor so it can mutate the engine's published chat directly.
