...
enum GoalTrackingEngine {
    ...
    /// The one progress fraction in the app. Internal rather than private because the guided setup's
    /// confirm ring and the goal widget draw the SAME number as this engine's snapshots — a second copy
    /// of this guard is how two surfaces end up disagreeing about one goal.
    static func progressFraction(goal: CoachGoal, current: Double?) -> Double? {
...
final class GoalTrackingStore: ObservableObject {
    ...
    /// What the app can currently measure, per goal KIND rather than per goal id — the same table
    /// `refresh` already builds for the snapshots, kept so a caller can ask about a kind that has no
    /// stored goal yet. The guided setup needs exactly that: its draft isn't saved when the confirm step
    /// wants to show where the user is starting from.
    @Published private(set) var measurementByKind: [CoachGoal.Kind: GoalMeasurement] = [:]

    func snapshot(for goalId: UUID) -> GoalTrackingSnapshot? {
        snapshots.first { $0.id == goalId }
    }

    func measurement(for kind: CoachGoal.Kind) -> GoalMeasurement? { measurementByKind[kind] }

    /// The goal to lead with wherever only ONE fits — the Today goals card, the iOS goal widget: the one
    /// that most needs attention (`health`), then the nearest target date. Lives here rather than inside
    /// either surface so the card and the widget can never lead with different goals.
    var primarySnapshot: GoalTrackingSnapshot? {
        snapshots
    ...
    func refresh(repo: Repository, now: Date,
        ...
        self.measurementByKind = measurementByKind
