...
enum AIProvider: String, CaseIterable, Identifiable {
    ...
    var modelOptions: [String] {
        ...
        case .openAI:
            ...
            // `max_completion_tokens` and no temperature (see `Providers/OpenAI.swift`). The cost
...
func performStreamingRequest(
    ...
    case 401, 403:
        throw AICoachError.badKey
    case 429:
        // The fork's `rateLimited` carries the countdown, so the streaming path reads `Retry-After`
        // exactly like the non-streaming one rather than throwing a bare "rate limited".
        throw AICoachError.rateLimited(retryAfter: retryAfterSeconds(http))
    default:
