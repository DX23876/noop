...
struct BodyClockDialCard: View {
    ...
    private struct DialGeometry {
        ...
        init(size: CGSize) {
            ...
            let rim: CGFloat = side / 2 - NoopMetrics.bodyClockDialRimInset
            let scale: CGFloat = rim / NoopMetrics.bodyClockDialTunedRim
            outer = rim
            labelRadius = rim - NoopMetrics.bodyClockDialLabelOffset * scale
            reference = rim - NoopMetrics.bodyClockDialReferenceOffset * scale
            night = rim - NoopMetrics.bodyClockDialNightOffset * scale
            glyphRadius = rim - NoopMetrics.bodyClockDialGlyphOffset * scale
        }
    }
    ...
    private var dial: some View {
        ...
        .frame(height: NoopMetrics.bodyClockDialHeight)
    }
    ...
    private func drawTicks(_ ctx: GraphicsContext, _ g: DialGeometry) {
        for tick in stride(from: 0.0, to: 24.0, by: 6.0) {
            ...
            let len: CGFloat = isMidnight ? NoopMetrics.bodyClockDialMidnightTick
                                          : NoopMetrics.bodyClockDialRegularTick
    ...
    private func drawHourLabels(_ ctx: GraphicsContext, _ g: DialGeometry) {
        for hour in stride(from: 0.0, to: 24.0, by: 6.0) {
            ...
            // A fixed diagram token, not `StrandFont.caption`. That is a text STYLE, so it scales with
            ...
            var numeral = ctx.resolve(
                Text(String(format: "%02d", Int(hour))).font(StrandFont.diagramLabel))
    ...
    private func drawArcs(_ ctx: GraphicsContext, _ g: DialGeometry) {
        if let ideal {
            strokeArc(ctx, g, radius: g.reference, from: ideal.bedHour, to: ideal.wakeHour,
                      colour: hue.opacity(0.55), width: NoopMetrics.bodyClockDialReferenceWidth,
                      dashed: true)
        }
        strokeArc(ctx, g, radius: g.night, from: actualBedHour, to: actualWakeHour,
                  colour: hue, width: NoopMetrics.bodyClockDialNightWidth, dashed: false)
    }
    ...
    private func strokeArc(_ ctx: GraphicsContext, _ g: DialGeometry, radius: CGFloat,
        ...
        let dash: [CGFloat] = dashed ? NoopMetrics.bodyClockDialReferenceDash : []
    ...
    private func drawOnset(_ ctx: GraphicsContext, _ g: DialGeometry) {
        ...
        let glyph: CGFloat = NoopMetrics.bodyClockDialGlyphSize
