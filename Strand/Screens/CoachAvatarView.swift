...
/// tinted disc, or the user's on-device photo clipped to a circle. Observes `CoachIdentityStore` so it
/// updates live wherever it's shown (chat header, Today entry, settings). Shared (macOS + iOS); the photo
/// bytes are loaded from Application Support and never leave the device.
